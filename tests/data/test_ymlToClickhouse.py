import pytest
from data.ymlToClickhouse import YMLtoClickhouse
...
def test_print_create_table():
    ...
    expected_result = (
        ...
        "  column2 String,\n"
        "  PRIMARY KEY (column1)\n"
        ") ENGINE = MergeTree ORDER BY (column1, column1);\n\n"
    )
    assert yml_to_ck.print_create_table(table_name, columns) == expected_result
...
def test_print_create_table_invalid_increment():
    ...
    with pytest.raises(ValueError) as excinfo:
        yml_to_ck.print_create_table(table_name, columns)
...
def test_print_insert_row():
    ...
    assert yml_to_ck.print_insert_row(
